					...
					if (newLevel != level) {
						...
						if (newLevel > level) {
							...
							if (rewards.levels.includes(newLevel)) {
								db.collection('perfis').doc(message.author.id).update({
									balance: bal + reward,
								}).then(() => message.channel.send(`🎉 ${lang.levelUp.congrats + message.author + lang.levelUp.levelTo + newLevel + lang.levelUp.received + reward}! 🆙💰`));
							}
							else {
								message.channel.send(`🎉 ${lang.levelUp.congrats + message.author + lang.levelUp.levelTo + newLevel}! 🆙`);
							}
						}
					}
				});
